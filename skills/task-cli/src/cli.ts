    ...
    try {
      ...
      for (const item of result.items) {
        ...
        const typedItem = item as typeof item & {
          blockers?: Array<{ id: number; displayId: string; title: string; status: string }>;
          blockerCount?: number;
        };
...
program
  ...
  .action(async (ids, options) => {
    try {
      ...
      if (options.parent) data.parentId = parseTaskId(options.parent) ?? parseInt(options.parent);

      // Handle --blocked-by: sets status to blocked AND adds the blockers
      if (options.blockedBy) {
        data.status = 'blocked' as ItemStatus;
      }
      ...
      // Parse blocker IDs
      const blockersToAdd: number[] = [];
      const blockersToRemove: number[] = [];

      if (options.blockedBy) {
        const ids = options.blockedBy.split(',').map((id: string) => parseTaskId(id.trim()) ?? parseInt(id.trim()));
        blockersToAdd.push(...ids);
      }
      if (options.addBlocker) {
        const ids = options.addBlocker.split(',').map((id: string) => parseTaskId(id.trim()) ?? parseInt(id.trim()));
        blockersToAdd.push(...ids);
      }
      if (options.removeBlocker) {
        const ids = options.removeBlocker.split(',').map((id: string) => parseTaskId(id.trim()) ?? parseInt(id.trim()));
  ...
  .action(async (type, title, options) => {
    try {
      ...
      const item = await trpc.items.create.mutate({
        ...
        status: options.blockedBy ? 'blocked' as const : undefined,
      });
      ...
      if (options.blockedBy) {
        const blockerIds = options.blockedBy.split(',').map((id: string) => parseTaskId(id.trim()) ?? parseInt(id.trim()));
  ...
  .action(async (name, options) => {
    try {
      if (name) {
        ...
        if (personDetail.org) console.log(`Org: ${personDetail.org}`);
        if (personDetail.email) console.log(`Email: ${personDetail.email}`);
        console.log('─'.repeat(40));
...
program
  ...
  .action(async (name, options) => {
    try {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const data: any = {};
      if (options.newName) data.name = options.newName;
      if (options.email) data.email = options.email;
      if (options.org) data.org = options.org;
...
routines
  ...
  .action(async (id) => {
    try {
      ...
      if (result.recurrenceTime) console.log(`Time:         ${result.recurrenceTime}`);
      if (result.recurrenceDays) console.log(`Days:         ${result.recurrenceDays}`);
...
routines
  ...
  .action(async (title, options) => {
    try {
      ...
      if (options.days) {
        recurrenceDays = options.days.split(',').map((d: string) => d.trim());
      }
...
routines
  ...
  .action(async (id, options) => {
    try {
      ...
      if (options.title) data.title = options.title;
      if (options.rule) data.recurrenceRule = options.rule;
      if (options.time) data.recurrenceTime = options.time;
      if (options.days) data.recurrenceDays = options.days.split(',').map((d: string) => d.trim());
      if (options.priority) data.priority = options.priority;
      if (options.description) data.description = options.description;
...
program
  ...
  .action(async (slug, options) => {
    try {
      ...
      if (options.newOrg) data.org = options.newOrg;
...
program
  ...
  .action(async (slug, options) => {
    try {
      ...
      let onItems: 'cascade' | 'orphan' | 'fail' = 'fail';
      let onChildren: 'cascade' | 'orphan' | 'fail' = 'fail';

      if (options.force) {
        onItems = 'cascade';
        onChildren = 'cascade';
      } else {
        if (options.cascadeItems) onItems = 'cascade';
        else if (options.orphanItems) onItems = 'orphan';

        if (options.cascadeChildren) onChildren = 'cascade';
        else if (options.orphanChildren) onChildren = 'orphan';
      }
  ...
  .action(async (fromId, linkType, toId) => {
    try {
      ...
      const result = await trpc.items.addLink.mutate({
        fromId,
        toId,
        linkType: linkType as 'blocks' | 'related' | 'duplicate',
      });
  ...
  .action(async (itemId, personName, role) => {
    try {
      ...
      const result = await trpc.items.addPerson.mutate({
        itemId,
        personName,
        role: role as 'assignee' | 'waiting_on' | 'stakeholder' | 'reviewer' | 'cc',
      });
...
tags
  ...
  .action(async (nameOrId) => {
    try {
      ...
      const result = await trpc.tags.get.query(
        isNumeric ? { id: parseInt(nameOrId) } : { name: nameOrId }
      );
...
tags
  ...
  .action(async (id, options) => {
    try {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const data: any = { id: tagId };
      if (options.name) data.name = options.name;
      if (options.color !== undefined) data.color = options.color || null;
      if (options.description !== undefined) data.description = options.description || null;
