...
describe('tRPC Organizations Router', () => {
    ...
    describe('organizations.get', () => {
        ...
        it('should throw NOT_FOUND for non-existent organization', async () => {
            await expect(caller.organizations.get({ slug: 'non-existent-org-xyz' })).rejects.toThrow(/not found/i);
        });
    });
    ...
    describe('organizations.update', () => {
        ...
        it('should throw NOT_FOUND for non-existent organization', async () => {
            await expect(caller.organizations.update({
    ...
    describe('organizations.delete', () => {
        it('should delete organization with no references', async () => {
            ...
            // Verify it's gone
            await expect(caller.organizations.get({ slug: tempSlug })).rejects.toThrow(/not found/i);
        });
        it('should reject deletion if projects reference it', async () => {
            // acme-corp has projects, so deletion should fail
            await expect(caller.organizations.delete({ slug: 'acme-corp' })).rejects.toThrow(/cannot delete/i);
        });
        it('should throw NOT_FOUND for non-existent organization', async () => {
            await expect(caller.organizations.delete({ slug: 'non-existent-org' })).rejects.toThrow(/not found/i);
        });
    });
