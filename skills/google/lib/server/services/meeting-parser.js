...
import { resolveKBPath, getRelativeKBPath } from './paths.js';
 ...
 */
export function parseActionTable(tableMarkdown, defaultProject) {
    const allLines = tableMarkdown.split('\n');
    // Detect if table has Project column by checking header row
    const headerLine = allLines.find(row => row.startsWith('|') && row.toLowerCase().includes('owner'));
    const hasProjectColumn = headerLine?.toLowerCase().includes('project') ?? false;
    const lines = allLines.filter(row => row.startsWith('|') && !row.includes('---') && !row.toLowerCase().includes('owner'));
 ...
 */
export function parseMeetingFile(filePath) {
    try {
 ...
 */
export function getAllMeetingFiles() {
    const files = [];
 ...
 */
export function parseAllMeetings() {
 ...
 */
export function getMeetingByPath(relativePath) {
