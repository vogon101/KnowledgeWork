...
import { ItemQuerySchema, CreateItemSchema, UpdateItemSchema, formatTaskId, } from '@kw/api-types';
...
import { resolveItemId } from '../utils/id-parser.js';
...
function formatItem(item) {
    return {
        ...
        projectOrgColor: item.project?.organization?.color || null,
...
export const itemsRouter = router({
    ...
    list: publicProcedure
        ...
        const { status, ownerId, ownerName, projectId, projectSlug, orgSlug, parentId, sourceMeetingId, itemType, dueBefore, dueAfter, targetPeriod, search, includeCompleted, limit = 100, offset = 0, } = input;
        // Build where clause
        const where = {
            deletedAt: null,
        };
        ...
        if (orgSlug)
            where.project = { ...where.project, organization: { slug: orgSlug } };
        // Relationship filters
        if (parentId)
            where.parentId = parentId;
        if (sourceMeetingId)
            where.sourceMeetingId = sourceMeetingId;
        // Date filters
    ...
    update: publicProcedure
        ...
        if (data.targetPeriod !== undefined)
        ...
        if (data.sourceMeetingId !== undefined)
            updateData.sourceMeetingId = data.sourceMeetingId;
    ...
    complete: publicProcedure
        ...
        let markdownSync = { synced: false, sourceType: null, message: '' };
        ...
        return {
            ...
            markdownSync,
            unblockedTasks: unblockedItems.map((t) => ({
                id: t.id,
