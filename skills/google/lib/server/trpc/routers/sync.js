...
import { getMeetingByPath, parseDueDate, } from '../../services/meeting-parser.js';
import { syncFilesystemToDb, syncWorkstreamToDb, syncItemToFile, detectAllConflicts, forceFileSyncToDb, forceDbSyncToFile, scanAllWorkstreams, scanWorkstreamFile, } from '../../services/file-sync.js';
// =============================================================================
...
export const syncRouter = router({
    ...
    meetingPreview: publicProcedure
        ...
        const actionsWithStatus = await Promise.all(meeting.actions.map(async (action) => {
...
// =============================================================================
async function syncMeetingActions(prisma, meeting) {
    const result = {
        meetingPath: meeting.path,
        meetingTitle: meeting.title,
        actionsFound: meeting.actions.length,
    ...
    for (const action of meeting.actions) {
        try {
            ...
            result.errors.push(`Failed to process action "${action.action}": ${error.message}`);
        }
    }
    return result;
}
