...
import { FileListInputSchema, FileTreeInputSchema, FileSearchInputSchema, FileGetInputSchema, } from '@kw/api-types';
import { resolveKBPath, getRelativeKBPath, isWithinKB, } from '../../services/paths.js';
 ...
 */
function shouldExclude(name, includeHidden) {
 ...
 */
function getFrontmatterTitle(absolutePath) {
    try {
 ...
 */
function toFileEntry(dirEntry, parentPath) {
 ...
 */
function searchFiles(dirPath, query, extensions, includeHidden, results, limit) {
    ...
    try {
        ...
        for (const entry of entries) {
            if (results.length >= limit)
                return;
            if (shouldExclude(entry.name, includeHidden))
                continue;
 ...
 */
function buildTree(absolutePath, relativePath, depth, includeHidden) {
    if (depth <= 0)
        return [];
    try {
