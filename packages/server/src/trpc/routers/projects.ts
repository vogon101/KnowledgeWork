...
function formatProject(project: {
  ...
  organization: { slug: string; name: string; shortName: string | null; color: string | null } | null;
}): ProjectWithParent {
  ...
  return {
    ...
    organizationColor: project.organization?.color &&
      (ORG_COLORS as readonly string[]).includes(project.organization.color)
        ? (project.organization.color as OrgColor)
        : null,
  };
}
...
export const projectsRouter = router({
  ...
  list: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (!includeChildren) where.parentId = null;
