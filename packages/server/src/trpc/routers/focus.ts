 ...
 */
function getFocusCSVPath(): string {
 ...
 */
function readFocusCSV(): FocusEntry[] {
  const csvPath = getFocusCSVPath();
 ...
 */
function writeFocusCSV(entries: FocusEntry[]): void {
  ensureFocusCSV();
  const csvPath = getFocusCSVPath();
...
export const focusRouter = router({
  ...
  summary: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      const weekGroups: Record<string, FocusEntry[]> = {};
