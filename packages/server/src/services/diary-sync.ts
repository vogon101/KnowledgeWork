 ...
 */
export function getDiaryPath(basePath: string, date: Date = new Date()): string {
 ...
 */
function createDiaryFile(filePath: string, date: Date): void {
  const year = date.getFullYear();
...
export function logTaskActivity(
  ...
  date: Date = new Date()
): { success: boolean; diaryPath: string; message: string } {
...
export function logRoutineCompletion(
  ...
  routineTitle: string,
  date: Date = new Date()
): { success: boolean; diaryPath: string } {
  const diaryPath = getDiaryPath(basePath, date);
