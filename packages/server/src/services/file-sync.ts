...
import { computeContentHash, detectConflict, type ConflictStatus } from './hash-utils.js';
...
export interface ScannedWorkstream {
  filePath: string;        // Relative path from knowledge base root
 ...
 */
function scanProjectForWorkstreams(
  projectPath: string,
  projectSlug: string,
  org: string
): ScannedWorkstream[] {
  ...
  if (!existsSync(projectPath)) return workstreams;
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      const workstreamFm = frontmatter as WorkstreamFrontmatter;
 ...
 */
export function scanAllWorkstreams(): ScannedWorkstream[] {
  const allWorkstreams: ScannedWorkstream[] = [];

  for (const org of ORGS) {
    const projectsPath = join(getKnowledgeBasePath(), org, 'projects');
    if (!existsSync(projectsPath)) continue;
 ...
 */
export function scanWorkstreamFile(filePath: string): ScannedWorkstream | null {
  ...
  try {
    ...
    console.error(`Error parsing ${absolutePath}: ${(error as Error).message}`);
    return null;
  }
}
 ...
 */
export async function syncWorkstreamToDb(workstream: ScannedWorkstream): Promise<{
 ...
 */
export async function syncFilesystemToDb(): Promise<SyncResult> {
  ...
  for (const workstream of workstreams) {
    try {
      ...
      result.errors.push(`Failed to sync ${workstream.filePath}: ${(error as Error).message}`);
    }
  }
 ...
 */
export async function syncItemToFile(itemId: number): Promise<ItemToFileResult> {
  const prisma = getPrisma();
 ...
 */
export async function detectAllConflicts(): Promise<ConflictInfo[]> {
  const prisma = getPrisma();
  ...
  for (const item of workstreamItems) {
    ...
    try {
      ...
      conflicts.push({
        filePath: item.filePath,
        itemId: item.id,
        reason: `Error reading file: ${(error as Error).message}`,
        fileHash: '',
        dbHash: item.fileHash,
      });
    }
 ...
 */
export async function forceFileSyncToDb(filePath: string): Promise<{
  success: boolean;
  itemId?: number;
  error?: string;
}> {
 ...
 */
export async function forceDbSyncToFile(itemId: number): Promise<ItemToFileResult> {
  const prisma = getPrisma();
  ...
  try {
    ...
    return { success: false, filePath: item.filePath, error: (error as Error).message };
  }
}
